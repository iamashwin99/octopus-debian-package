...
oct-vibrational_spectrum - Calculates the vibrational spectrum from a molecular-dynamics run.

=head1 SYNOPSIS

=over

=item B<oct-vibrational_spectrum>

=back

[oct-vibrational_spectrum does not read the standard input: all standard
input will be simply ignored. An input file named F<inp> must be
present in the running directory.  Also, oct-vibrational_spectrum accepts
...
It computes a vibrational spectrum from a molecular-dynamics run, either by analyzing just the atomic motions, or also by considering the dipole moment to get an infrared spectrum.
