...
void FC_FUNC_(oct_mkdir, OCT_MKDIR)(STR_F_TYPE name STR_ARG1) {
  struct stat buf;
  char *name_c;
...
void FC_FUNC_(oct_stat, OCT_STAT)(fint *ierr, STR_F_TYPE name,
                                  STR_F_TYPE mod_time STR_ARG2) {
  ...
  *ierr = stat(name_c, &statbuf);
  ...
  /* otherwise, do not do this since 'mod_time_c' points at static data of
   * asctime */
}

int FC_FUNC_(oct_dir_exists, OCT_DIR_EXISTS)(STR_F_TYPE name STR_ARG1) {
  int ierr;
  char *name_c;
  struct stat statbuf;
...
void FC_FUNC_(oct_rm, OCT_RM)(STR_F_TYPE name STR_ARG1) {
  char *name_c;
...
void FC_FUNC_(oct_getcwd, OCT_GETCWD)(STR_F_TYPE name STR_ARG1) {
  char s[256];
  getcwd(s, 256);
  TO_F_STR1(s, name);
}

void FC_FUNC_(oct_realpath, OCT_REALPATH)(STR_F_TYPE fnam,
                                          STR_F_TYPE rnam STR_ARG2) {
...
void FC_FUNC_(oct_dirname, OCT_DIRNAME)(STR_F_TYPE fnam,
                                        STR_F_TYPE dnam STR_ARG2) {
...
void FC_FUNC_(oct_basename, OCT_BASENAME)(STR_F_TYPE fnam,
                                          STR_F_TYPE bnam STR_ARG2) {
...
void FC_FUNC_(oct_getenv, OCT_GETENV)(STR_F_TYPE var,
                                      STR_F_TYPE value STR_ARG2) {
  char *name_c, *var_c;
...
void FC_FUNC_(oct_wfs_list, OCT_WFS_LIST)(STR_F_TYPE str,
                                          fint l[16384] STR_ARG1) {
  int i, i1, i2;
...
void FC_FUNC_(oct_progress_bar, OCT_PROGRESS_BAR)(fint *a, fint *max) {
  if (*max == 0)
    return; /* Skip the bar if the length is 0 */
  progress_bar(*a, *max);
}

/* ------------------------------ some stuff  --------------------------------
 */
void FC_FUNC_(oct_gettimeofday, OCT_GETTIMEOFDAY)(fint *sec, fint *usec) {
  ...
  /* The typecast below should use long. However, this causes incompatibilities
     ...
     Using int will cause wrong results when tv.tv_sec exceeds
     INT_MAX=2147483647 */
  *sec = (int)tv.tv_sec;
  *usec = (int)tv.tv_usec;
/*
...
void FC_FUNC_(oct_nanosleep, OCT_NANOSLEEP)(fint *sec, fint *nsec) {
...
void FC_FUNC_(oct_sysname, OCT_SYSNAME)(STR_F_TYPE name STR_ARG1) {
  char *name_c;
...
int FC_FUNC_(oct_number_of_lines,
             OCT_NUMBER_OF_LINES)(STR_F_TYPE name STR_ARG1) {
...
void FC_FUNC_(oct_break_c_string,
              OCT_BREAK_C_STRING)(char **str, char **s,
                                  STR_F_TYPE line_f STR_ARG1) {
...
This function searches in directory given by dirname all files that have the
following name:

*_<real_number>_<integer>*

It returns the value of <real_number> found that is closest to freq (or
abs(freq)) and for which the value of <integer> matches with the tag argument.
...
void FC_FUNC_(oct_search_file_lr,
              OCT_SEARCH_FILE_LR)(double *freq, const fint *tag, fint *ierr,
                                  STR_F_TYPE dirname STR_ARG1) {
#if defined(HAVE_DIRENT_H) && defined(HAVE_CLOSEDIR) &&                        \
    defined(HAVE_READDIR) && defined(HAVE_STRCHR) && defined(HAVE_STRTOD)
  ...
  while (1) {
    ...
    if (num_start != NULL) {
      ...
      if (!found_something) {
        min = read_value;
        found_something = 1;
      } else if (fabs(fabs(min) - fabs(*freq)) >
                 fabs(fabs(read_value) - fabs(*freq))) {
...
#warning directory search not compiled
  fprintf(stderr, "Warning: Directory search not available since certain C "
...
void FC_FUNC_(oct_exit_failure, OCT_EXIT_FAILURE)() { exit(EXIT_FAILURE); }
