...
/* --------------------------------------------------------- */
void FC_FUNC_(varinfo_init, VARINFO_INIT)(STR_F_TYPE const fname STR_ARG1) {
...
/* --------------------------------------------------------- */
void FC_FUNC_(varinfo_getvar, VARINFO_GETVAR)(STR_F_TYPE const name,
                                              var_type **var STR_ARG1) {
  char *name_c;
  var_type *lvar;

  TO_C_STR1(name, name_c);
  for (lvar = vars; (lvar != NULL) && (strcasecmp(name_c, lvar->name) != 0);
       lvar = lvar->next)
    ;
  free(name_c);
...
/* --------------------------------------------------------- */
void FC_FUNC_(varinfo_getinfo, VARINFO_GETINFO)(const var_type **var,
                                                char **name, char **type,
                                                char **default_str,
                                                char **section, char **desc) {
  if (var == NULL) {
    *name = NULL;
    *type = NULL;
    *default_str = NULL;
...
/* --------------------------------------------------------- */
void FC_FUNC_(varinfo_getopt, VARINFO_GETOPT)(const var_type **var,
                                              opt_type **opt) {
  if (*var == NULL)
    *opt = NULL;
  else if (*opt == NULL)
...
void FC_FUNC_(varinfo_opt_getinfo,
              VARINFO_OPT_GETINFO)(const opt_type **opt, char **name,
                                   fint8 *value, char **desc) {
  if (opt == NULL) {
    *name = NULL;
    *desc = NULL;
    *value = 0;
...
int varinfo_variable_exists(const char *var_name) {
  var_type *lvar;
  for (lvar = vars; (lvar != NULL) && (strcasecmp(var_name, lvar->name) != 0);
       lvar = lvar->next)
    ;
  return (lvar != NULL);
}

void FC_FUNC_(varinfo_search_var, VARINFO_SEARCH_VAR)(const STR_F_TYPE name,
                                                      var_type **var STR_ARG1) {
  char *name_c;
  var_type *lvar;
  ...
  TO_C_STR1(name, name_c);
  for (; (lvar != NULL) && (strcasestr(lvar->name, name_c) == 0);
       lvar = lvar->next)
    ;
  free(name_c);
...
void FC_FUNC_(varinfo_search_option,
              VARINFO_SEARCH_OPTION)(const var_type **var,
                                     const STR_F_TYPE name, int *value,
                                     int *ierr STR_ARG1) {
  char *name_c;
  opt_type *opt;
