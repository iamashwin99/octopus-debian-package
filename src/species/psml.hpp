...
public:
  psml(const std::string &filename, bool uniform_grid = false)
    ...
    bool has_semilocal_potentials =
        root_node_->first_node("semilocal-potentials");
    bool has_pseudo_wavefunctions =
        root_node_->first_node("pseudo-wave-functions");
  ...
  int nprojectors() const {
    rapidxml::xml_node<> *node =
        root_node_->first_node("nonlocal-projectors")->first_node("proj");
    int count = 0;
    while (node) {
      count++;
      node = node->next_sibling("proj");
  ...
  void radial_function(int l, std::vector<double> &val) const {
    rapidxml::xml_node<> *node =
        root_node_->first_node("pseudo-wave-functions")->first_node("pswf");
    while (node) {
      int read_l = letter_to_l(node->first_attribute("l")->value());
      if (l == read_l)
        break;
  ...
  void radial_potential(int l, std::vector<double> &val) const {
    rapidxml::xml_node<> *node =
        root_node_->first_node("semilocal-potentials")->first_node("slps");
    while (node) {
      int read_l = letter_to_l(node->first_attribute("l")->value());
      if (l == read_l)
        break;
