...
contains
  ...
  subroutine td_etrs(ks, namespace, space, hm, ext_partners, gr, st, tr, time, dt, &
    ionic_scale, ions_dyn, ions, move_ions)
    ...
    if (hm%theory_level /= INDEPENDENT_PARTICLES) then
      ...
      if (family_is_mgga_with_exc(hm%xc)) then
        SAFE_ALLOCATE(vtau_t1(1:gr%mesh%np, 1:st%d%nspin))
        SAFE_ALLOCATE(vtau_t2(1:gr%mesh%np, 1:st%d%nspin))
        call lalg_copy(gr%mesh%np,  st%d%nspin, hm%vtau, vtau_t1)
      end if
      ...
      if(family_is_mgga_with_exc(hm%xc)) then
        call lalg_copy(gr%mesh%np, st%d%nspin, hm%vtau, vtau_t2)
        call lalg_copy(gr%mesh%np, st%d%nspin, vtau_t1, hm%vtau)
      end if
      call hamiltonian_elec_update(hm, gr%mesh, namespace, space, ext_partners, time = time - dt)
    ...
    call propagation_ops_elec_move_ions(tr%propagation_ops_elec, gr, hm, st, namespace, space, ions_dyn, ions, &
      ext_partners, time, ionic_scale*dt, move_ions = move_ions)
    ...
    if (hm%theory_level /= INDEPENDENT_PARTICLES) then
      ...
      if(family_is_mgga_with_exc(hm%xc)) then
        call lalg_copy(gr%mesh%np, st%d%nspin, vtau_t2, hm%vtau)
      end if
    end if
  ...
  subroutine td_etrs_sc(ks, namespace, space, hm, ext_partners, gr, st, tr, time, dt, &
    ionic_scale, ions_dyn, ions, move_ions, sctol, scsteps)
    type(v_ks_t),                     intent(inout) :: ks
    type(namespace_t),                intent(in)    :: namespace
    type(space_t),                    intent(in)    :: space
    type(hamiltonian_elec_t),         intent(inout) :: hm
  ...
  subroutine td_aetrs(namespace, space, hm, gr, st, tr, time, dt, ionic_scale, ions_dyn, ions, ext_partners, move_ions)
    type(namespace_t),                intent(in)    :: namespace
    type(space_t)    ,                intent(in)    :: space
    type(hamiltonian_elec_t),         intent(inout) :: hm
    type(grid_t),                     intent(in)    :: gr
    ...
    call propagation_ops_elec_move_ions(tr%propagation_ops_elec, gr, hm, st, namespace, space, ions_dyn, &
      ions, ext_partners, time, ionic_scale*dt, move_ions = move_ions)
  ...
  ! ---------------------------------------------------------
  !> Propagator with approximate enforced time-reversal symmetry
  subroutine td_caetrs(ks, namespace, space, hm, ext_partners, gr, st, tr, time, dt, ionic_scale, &
      ...
      do ib = st%group%block_start, st%group%block_end
        ...
        case (BATCH_DEVICE_PACKED)
          ...
          call accel_kernel_run(kernel_phase, (/st%group%psib(ib, ik)%pack_size(1), dim2, dim3/), &
            (/st%group%psib(ib, ik)%pack_size(1), wgsize, 1/))
