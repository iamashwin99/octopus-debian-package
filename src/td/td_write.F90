...
contains
  ...
  subroutine td_write_iter(writ, namespace, space, outp, gr, st, hm, ions, ext_partners, kick, ks, dt, iter)
    ...
    if (writ%out(OUT_FTCHD)%write) then
      call td_write_ftchd(writ%out(OUT_FTCHD)%handle, space, gr%mesh, st, kick, iter)
    end if
    ...
    if (writ%out(OUT_SPIN)%write) then
      call td_write_spin(writ%out(OUT_SPIN)%handle, gr%mesh, st, iter)
    end if
    ...
    if (writ%out(OUT_TOT_M)%write) then
      call td_write_tot_mag(writ%out(OUT_TOT_M)%handle, gr%mesh, st, kick, iter)
    end if
    ...
    if (writ%out(OUT_PROJ)%write .and. mod(iter, writ%compute_interval) == 0) then
      ...
      call td_write_proj(writ%out(OUT_PROJ)%handle, space, gr%mesh, ions, st, writ%gs_st, kick, iter)
    end if

    if (writ%out(OUT_FLOQUET)%write) then
      call td_write_floquet(namespace, space, hm, ext_partners, gr%mesh, st, iter)
    end if

    if (writ%out(OUT_KP_PROJ)%write) then
      call td_write_proj_kp(gr%mesh, hm%kpoints, st, writ%gs_st, namespace, iter)
    end if
    ...
    if (writ%out(OUT_ION_CH)%write) then
      call td_write_ionch(writ%out(OUT_ION_CH)%handle, gr%mesh, st, iter)
    end if

    if (writ%out(OUT_TOTAL_CURRENT)%write) then
      call td_write_total_current(writ%out(OUT_TOTAL_CURRENT)%handle, space, gr%mesh, st, iter)
    end if
    ...
    if (writ%out(OUT_N_EX)%write .and. mod(iter, writ%compute_interval) == 0) then
      ...
      call td_write_n_ex(writ%out(OUT_N_EX)%handle, outp, namespace, gr%mesh, hm%kpoints, st, writ%gs_st, iter)
    end if
  ...
  subroutine td_write_output(namespace, space, gr, st, hm, ks, outp, ions, ext_partners, iter, dt)
    ...
    if (present(dt)) then
      call output_scalar_pot(outp, namespace, space, filename, gr%mesh, ions, ext_partners, iter*dt)
    else
      if (iter == 0) call output_scalar_pot(outp, namespace, space, filename, gr%mesh, ions, ext_partners)
    end if
  ...
  subroutine td_write_angular(out_angular, namespace, space, gr, ions, hm, st, kick, iter)
    ...
    call pert_init(angular_momentum, namespace, PERTURBATION_MAGNETIC, ions)

    do idir = 1, 3
      call pert_setup_dir(angular_momentum, idir)
      !we have to multiply by 2, because the perturbation returns L/2
      angular(idir) = &
        M_TWO*TOFLOAT(zpert_states_elec_expectation_value(angular_momentum, namespace, space, gr, hm, st))
    end do
  ...
  subroutine td_write_multipole(out_multip, space, gr, ions, st, lmax, kick, iter)
    ...
    if (out_multip%resolve_states) then
      ...
      do ist = st%st_start, st%st_end
        ...
        call td_write_multipole_r(out_multip%mult_handles(ist), space, gr%mesh, ions, st, lmax, kick, rho, iter, &
      ...
      call td_write_multipole_r(out_multip%handle, space, gr%mesh, ions, st, lmax, kick, st%rho, iter)
  ...
  subroutine td_write_floquet(namespace, space, hm, ext_partners, mesh, st, iter)
    ...
    do it = 1, nT
      ...
      call hamiltonian_elec_update(hm, mesh, namespace, space, ext_partners, time=Tcycle+it*dt)
      ! get hpsi
    ...
    call hamiltonian_elec_update(hm, mesh, namespace, space, ext_partners, time=M_ZERO)
  ...
  ! ---------------------------------------------------------
  subroutine td_write_mxll_iter(writ, space, gr, st, hm, dt, iter)
    type(td_write_t),              intent(inout) :: writ
    type(space_t),                 intent(in)    :: space
