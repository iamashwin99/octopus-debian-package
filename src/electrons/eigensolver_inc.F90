...
subroutine X(eigensolver_run)(eigens, namespace, mesh, st, hm, iter, ik)
  ...
  !> Local copy of max subspace solver iterations
  integer :: maxiter
  ...
  case(RS_CHEBYSHEV)
    call X(chebyshev_filter_solver) (namespace, eigens%sdiag, mesh, st, hm, ik, eigens%tolerance, &
      eigens%cheby_params, iter)
  end select
