...
contains
  ...
  contains
    ...
    subroutine out_basins(ff, filename, output_how)
      ...
      call basins_init(basins, namespace, gr%mesh)
      call basins_analyze(basins, namespace, gr%mesh, ff(:), st%rho, CNST(0.01))
  ...
  subroutine output_berkeleygw(bgw, namespace, space, dir, st, gr, ks, hm, ions)
    ...
    call cube_init(cube, gr%mesh%idx%ll, namespace, space, gr%mesh%spacing, gr%mesh%coord_system, &
    ...
    call fourier_shell_init(shell_density, namespace, space, cube, gr%mesh)
    ...
    do ik = st%d%kpt%start, st%d%kpt%end, st%d%nspin
      call fourier_shell_init(shell_wfn, namespace, space, cube, gr%mesh, kk = hm%kpoints%reduced%red_point(:, ik))
...
#include "output_linear_medium_inc.F90"
