...
subroutine output_mxll(outp, namespace, space, gr_mxll, st_mxll, hm_mxll, time, dir, gr_elec, st_elec, hm_elec)
  ...
  call output_states_mxll(outp, namespace, space, dir, st_mxll, gr_mxll%mesh, hm_mxll)
  call output_energy_density_mxll(outp, namespace, space, dir, hm_mxll, gr_mxll%mesh)
  call output_poynting_vector_orbital_angular_momentum(outp, namespace, space, dir, st_mxll, gr_mxll%mesh, hm_mxll)
  call output_transverse_rs_state(outp, namespace, space, dir, st_mxll, gr_mxll%mesh)
  call output_longitudinal_rs_state(outp, namespace, space, dir, st_mxll, gr_mxll%mesh)
  call output_divergence_rs_state(outp, namespace, space, dir, st_mxll, gr_mxll)
  call output_external_current_density(outp, namespace, space, dir, st_mxll, gr_mxll%mesh, hm_mxll, time)
...
subroutine output_poynting_vector_orbital_angular_momentum(outp, namespace, space, dir, st, mesh, hm)
 ...
 !----------------------------------------------------------
subroutine output_transverse_rs_state(outp, namespace, space, dir, st, mesh)    !< have to set unit output correctly
  type(output_t),            intent(in)    :: outp
  type(namespace_t),         intent(in)    :: namespace
  type(space_t),             intent(in)    :: space
  character(len=*),          intent(in)    :: dir
  ...
  if (outp%what(OPTION__MAXWELLOUTPUT__TRANS_ELECTRIC_FIELD)) then
    message(1) = 'Transverse field is equal to total field at the moment.'
    call messages_warning(1)
    fn_unit = units_out%energy/units_out%length
    SAFE_ALLOCATE(dtmp(1:mesh%np, 1:mesh%box%dim))
  ...
  if (outp%what(OPTION__MAXWELLOUTPUT__TRANS_MAGNETIC_FIELD)) then
    ...
    fn_unit = unit_one/units_out%length**2
    SAFE_ALLOCATE(dtmp(1:mesh%np, 1:mesh%box%dim))
    call get_magnetic_field_state(st%rs_state_trans, mesh, st%rs_sign, dtmp, st%mu(1:mesh%np), mesh%np)
    fname = 'b_field_trans'
...
subroutine output_longitudinal_rs_state(outp, namespace, space, dir, st, mesh)    !< have to set unit output correctly
  type(output_t),            intent(in)    :: outp
  type(namespace_t),         intent(in)    :: namespace
  type(space_t),             intent(in)    :: space
  character(len=*),          intent(in)    :: dir
  ...
  if (outp%what(OPTION__MAXWELLOUTPUT__LONG_ELECTRIC_FIELD)) then
    message(1) = 'Longitudinal output is not yet implemented, will be operational soon.'
...
subroutine output_divergence_rs_state(outp, namespace, space, dir, st, gr)    !< have to set unit output correctly
  ...
  if (outp%what(OPTION__MAXWELLOUTPUT__DIV_ELECTRIC_FIELD)) then
    ...
    SAFE_ALLOCATE(dtmp_1(1:gr%mesh%np_part, 1:gr%mesh%box%dim))
    SAFE_ALLOCATE(dtmp_2(1:gr%mesh%np))
    dtmp_1 = M_ZERO
    call get_electric_field_state(st%rs_state, gr%mesh, dtmp_1(1:gr%mesh%np,:), st%ep(1:gr%mesh%np), gr%mesh%np)
