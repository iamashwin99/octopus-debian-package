...
contains
  ...
  subroutine dielectric_function_compute()
    ...
    integer :: in_file, out_file, ref_file, ii, jj, kk, idir, jdir
    ...
    do kk = 1, energy_steps
      ...
      chi(1:space%dim, kk) = (dielectric(1:space%dim, kk) - vecpot0(1:space%dim)/n0)*latt%rcell_volume/(M_FOUR*M_PI)
    end do
    ...
    write(out_file,'(a)') trim(header)
    do kk = 1, energy_steps
      dielectric(1:space%dim, kk) = (dielectric(1:space%dim, kk) - M_ONE)/(M_FOUR*M_PI)
      ww = (kk-1)*spectrum%energy_step + spectrum%min_energy
      write(out_file, '(e15.6)', advance='no') ww
      do idir = 1, space%dim
